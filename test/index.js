...
test('fail invalid signaturecertchainurl header', function(t) {
  var mockRes = invokeMiddleware({
    ...
    t.equal(mockRes.statusCode, 400)
    t.deepEqual(JSON.parse(mockRes._getData()), {
      reason: 'Certificate URI hostname must be cortanaserviceapitranslatorbot.trafficmanager.net: invalid',
      status: 'failure'
    })
...
// TODO: FIX THIS (related to https://github.com/mreinstein/cortana-verifier/issues/18)?
/* test('fail invalid signature', function(t) {
  var mockRes = invokeMiddleware({
    headers: {
      signature: 'aGVsbG8NCg==',
      signaturecertchainurl: 'https://cortanaserviceapitranslatorbot.trafficmanager.net/certificate/SkillsApiCertificate.cer'
    },
    body: JSON.stringify({
      request: {
        timestamp: new Date().getTime()
